...
記事を書く人（Writer-Tech）の入口。**見せ場 → 該当コード（ファイル:行）→ スクショ／ログ**の対応表と、スパイクで詰まった点のうち記事向きのものをまとめる。記事プランそのものは [SPEC.md §8](../SPEC.md)。
...
## 4. 記事に出してはいけないもの（SPEC §8-3・人間指示 2026-09-12）

- 実運用の DB・項目・author 名。記事の author は **`ai:demo-assistant`** のみ
- タスク ID（`t-xxxx`）・チーム内のメンバー名・`.ai-team/` の話
